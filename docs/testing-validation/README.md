...
#### Resolution: 
Have those empty wire spaces be treated as such when the golden sample is submitted so that theyre saved as not existn meaning when a test harness is connected those wire positions are treated as not part of the test and input is 0 and no error message is displayed.
